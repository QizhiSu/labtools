...
export4msfinder <- function(data,
  ...
  if (all(column_names %in% colnames(data)) == FALSE)
    stop('Your data does not have Name, InChIKey, CID, ExactMass, Formula, and/or SMILES ',
  ...
  data <-
    data %>%
    dplyr::mutate(Short_InChIKey = sub(".{13}$", "", InChIKey),
...
export4msdial <- function(data,
  ...
  if (all(column_names %in% colnames(data)) == FALSE)
    stop('Your data does not have Name and ExactMass columns',
  ...
  if(polarity == 'pos') {
    ...
    data <-
      ...
      mutate(MZ = ifelse(Adduct == '[M+H]+', MZ + 1.007276, MZ),
